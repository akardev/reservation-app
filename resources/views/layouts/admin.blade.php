...
<head>
    ...
    <title>ADMIN PANEL | AKARDEV</title>
        ...
        <div @click.away="open = false"
            ...
            <div class="flex flex-row items-center justify-between flex-shrink-0 px-8 py-4">
                <a href="{{ route('admin.index') }}"
                    class="text-lg font-semibold tracking-widest text-gray-900 uppercase rounded-lg dark:text-white focus:outline-none focus:shadow-outline">AKARDEV</a>
            ...
            <div>
                ...
                @if (session()->has('danger'))
                    <div class="p-4 mb-4 text-sm text-red-700 bg-red-100 rounded-lg dark:bg-red-200 dark:text-red-800"
                        role="alert">
                        <span class="font-medium"> Bilgi!</span> {{ session()->get('danger') }}
                    </div>
                @endif
                ...
                @if (session()->has('success'))
                    <div class="p-4 mb-4 text-sm text-green-700 bg-green-100 rounded-lg dark:bg-green-200 dark:text-green-800"
                        role="alert">
                        <span class="font-medium">Bilgi!</span> {{ session()->get('success') }}
                    </div>
                @endif

                @if (session()->has('info'))
                    <div class="p-4 mb-4 text-sm text-blue-700 bg-blue-500 rounded-lg dark:bg-blue-300 dark:text-blue-800"
                        role="alert">
                        <span class="font-medium">Bilgi!</span> {{ session()->get('info') }}
                    </div>
                @endif
                ...
                @if (session()->has('warning'))
                    <div class="p-4 mb-4 text-sm text-yellow-700 bg-yellow-100 rounded-lg dark:bg-yellow-200 dark:text-yellow-800"
                        role="alert">
                        <span class="font-medium">Bilgi!</span> {{ session()->get('warning') }}
                    </div>
                @endif
